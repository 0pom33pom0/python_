                ...
                if x[7]!=0:
                    for i in range(4):
                        if money_d[i] == x[7]:
                            ...
                            delta = l_date - f_date
                            numday = delta.days
                            sum_1 = x[7]+((x[7]*dog[i-1]*numday)/30)
        ...
        if c.fetchone():
            ...
            if (code1 == code):
                ...
                try:
                    ...
                    continue
                c = conn.cursor()
                data = (users,code,name,address_n+"/"+address_sw+" ตำบล "+district_t+" อำเภอ "+district_o+" จังหวัด "+city,"0"+str(tel),0,0,0,0)
...
def delete():
    de = input("กรุณากรอกUsersnameที่ท่านต้องการลบ:")
    de_t = (de,)
    c = conn.cursor()
    c.execute(' SELECT * FROM loan WHERE Users = ? ',de)
    if c.fetchone():
        print("1:ลบดอก\n2:ลบต้นและดอก")
        kk = input("เลือกรายการ:")
        if kk == "1":
            now = datetime.datetime.now()
            Y = "%d" % now.year
            M = "%d" % now.month
            D = "%d" % now.day
            data = (int(Y),int(M),int(D),de,)
            c = conn.cursor()
            c.execute('''UPDATE loan SET Year = ?,Month = ?,Day = ? WHERE Users = ?''',data)
            conn.commit()
            c.close()
            print("🤑🤑ลบเรียบร้อยครับนายท่าน!🤑🤑")
            time.sleep(3)
        else:
            data = (0,0,0,0,de,)
            c = conn.cursor()
        ...
        print("!!!😖ผมหาUsersนี้ไม่เจอครับนายท่าน😖!!!")
        time.sleep(2)
...
while(True):
    ...
    if yn == 1:
        ...
        while(True):
            print("🙏!!!!ยืนดีต้อนรับครับนายท่าน!!!!🙏\n🤔ท่านต้องการทำสิ่งใด🤔\n1:ลบจำนวนเงินลูกค้า\n2:ออกจากระบบ!")
            okhub = int(input("เลือกเลยครับนายท่าน:"))
            if okhub == 1:
                delete()
            else:
                break
